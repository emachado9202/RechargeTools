...
namespace RechargeTools.Async
{
    public partial class LocalAsyncState : IAsyncState
    {
        private readonly MemoryCache _states = new MemoryCache("RechargeTools.AsyncState.Progress");
        private readonly MemoryCache _cancelTokens = new MemoryCache("RechargeTools.AsyncState.CancelTokenSources");

        public virtual bool Exists<T>(string name = null)
        {
            var value = GetStateInfo<T>(name);
            return value != null && !Equals(value.Progress, default(T));
        }
